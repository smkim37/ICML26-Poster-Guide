...
- [ ] `lib/time.ts` (Asia/Seoul, `?now=` 오버라이드) + `useNow`(30초 갱신) — docs/02 §7 스펙 그대로
- [ ] NowBanner 4상태 (D-n / 지금 세션 / 다음 세션 / 종료 후 숨김) — docs/01 §4
- [ ] SchedulePage: docs/05 내용으로 7/5–7/11 일자별 가이드 + 포스터 세션 표 + 키노트
- [ ] StatsPage: StatTile(방문 n/137, 핵심 n/30, 별표, 메모) + StatBar 분포 6종 + ICML 전체 통계

**Acceptance**
- [ ] `?now=2026-07-08T15:00` → "지금 Poster Session 4" / `?now=2026-07-08T13:00` → 다음 세션 카운트다운 / `?now=2026-06-20T10:00` → D-17
- [ ] 통계 수치가 30/52/55, 41/35/46/15, 79/58, 19/14/11/9와 일치
- [ ] NowBanner의 "지금" 상태에서 탭 → 해당 세션으로 스크롤
