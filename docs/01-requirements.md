...
앱은 공개 공유되지만 개인화 기능은 소유자 전용. 강력한 보안이 아닌 **노출 방지용 간단 게이트**.

- 해제: 설정 페이지 "개인 모드" 섹션에서 비밀번호 입력 → SHA-256 해시 비교 (`src/hooks/usePersonalMode.tsx`의 `PW_HASH`; 평문은 코드에 없음)
- 유지: 해제 후 **5분** (`PERSONAL_TTL_MS`), `icml26.personal.v1`에 만료시각 저장. 만료 시 자동 잠금(타이머 + visibilitychange 재검사). 수동 "잠금" 버튼 제공
- 잠금/만료돼도 userdata는 삭제되지 않음 — UI만 숨김, 재해제 시 복귀

### 방문자(잠금 상태)에게 숨기는 것

| 위치 | 숨김 |
|---|---|
| PosterCard | 체크·별표 버튼, 방문 흐림 스타일 |
| SessionGroup | 진행률 x/y·ProgressBar (→ `N편` 표기) |
| PaperPage | 방문/별표 버튼, MemoEditor |
| FilterSheet | "미방문만" 칩 |
| NowBanner | "미방문 핵심 k편" → "핵심 k편(총수)" |
| StatsPage | "관람 진행률" 섹션 전체 |
| SettingsPage | 백업·초기화 섹션 (→ 개인 모드 해제 UI 표시) |
| AppHeader | (해제 시에만) 우측 액센트 도트 표시 |

공개 유지: 리스트·검색·필터(방문 여부 제외)·상세·일정·분포/ICML 통계·테마.
