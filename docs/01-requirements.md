...
앱은 링크로 공개 공유되며 **UI는 모든 방문자에게 동일하게 보인다**. 다만 개인화 *액션*은
소유자 전용 — 잠금 상태에서 시도하면 안내 팝업이 뜬다. 팝업은 단계 칩
`설정 → 개인 모드 → 비밀번호 입력` + [닫기 / 설정으로 이동] 버튼으로 구성되고,
"설정으로 이동"은 설정 탭 **최상단**으로 스크롤한다 (개인 모드가 설정의 첫 섹션).
강력한 보안이 아닌 노출·오조작 방지용 간단 게이트.
...
- 게이트 진입점은 `usePersonalMode().requirePersonal()` 하나 — 개인화 액션 핸들러
  첫 줄에서 호출하고 false면 중단(팝업은 Provider가 렌더)

### 잠금 상태에서 팝업으로 막는 액션

| 위치 | 게이트되는 액션 |
|---|---|
| PosterCard / PaperPage | 방문 체크·별표 토글 |
| MemoEditor (PaperPage) | 메모 입력(focus 시점에 차단) |
| FilterSheet | "미방문만" 칩 토글 |
| SettingsPage | 백업 내보내기·가져오기, 데이터 초기화 |

읽기 전용 표시(진행률 x/y, 통계 "관람 진행률", NowBanner 문구)는 게이트 없이 모두에게
렌더된다 — 방문자에게는 0으로 보일 뿐이다. AppHeader 우측 액센트 도트는 해제 상태 표시.
